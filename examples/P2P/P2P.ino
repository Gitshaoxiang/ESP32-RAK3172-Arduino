...
void updateStatus(String product, String snr, String rssi, String data,
                  String count) {
    canvas.clear();
...
void updateUI() {
    ...
    updateConfig(String(LORA_FREQ_STR) + " SF: " + String(sf) +
                 " BW: " + String(bw) + "K " + " CR: 4/" + String(cr + 5));
...
void setup() {
    ...
    if (lora.config(LORA_FREQ, sf, bw, cr, LORA_CONFIG_PRLEN,
                    LORA_CONFIG_PWR)) {
        updateUI();
        beep();
    }
...
void loop() {
    ...
    if (M5.BtnA.wasReleased()) {
        ...
        if (lora.print(message.c_str())) {
            ...
            updateStatus(PRODUCT_NAME, snr_str, rssi_str, incoming_str,
                         sent_count_str);
            msgCount++;
        }
    }
